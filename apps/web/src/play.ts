...
// Templates: arena, runner, racer, merge (2048), logic (lights-out).
...
function startGame() {
  state = 'playing'; ov.classList.add('hidden'); A.setScore(0); game.init();
}
function endGame(s) {
  state = 'over';
...
function loop(t) {
  ...
  if (state === 'playing') game.update(dt);
  game.render();
...
function makeLogic() {
  ...
  function layout() { var s = Math.min(A.W, A.H) - 48; gap = Math.max(7, s * 0.03); tile = (s - gap * (n - 1)) / n; ox = (A.W - (tile * n + gap * (n - 1))) / 2; oy = (A.H - (tile * n + gap * (n - 1))) / 2; }
  function resize() { layout(); }
  ...
  function pointerdown(x, y) { var gx = Math.floor((x - ox) / (tile + gap)), gy = Math.floor((y - oy) / (tile + gap)); if (gx < 0 || gy < 0 || gx >= n || gy >= n) return; if ((x - ox) - gx * (tile + gap) > tile || (y - oy) - gy * (tile + gap) > tile) return; flip(gy * n + gx); if (solved()) { level++; build(); } }
  function render() {
    var c = A.ctx; bg();
...
var FACTORIES = { arena: makeArena, runner: makeRunner, racer: makeRacer, merge: makeMerge, logic: makeLogic };
...
export function playDoc(o: { slug: string; title: string; accent: string; template: string }): string {
  const theme = JSON.stringify({ slug: o.slug, title: o.title, accent: o.accent, template: o.template });
